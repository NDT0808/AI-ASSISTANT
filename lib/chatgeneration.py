...
def generate_chat_completion(model, messages, temp=0, max_num_tokens=1000):
    ...
    """
    # ✅ Kiểm tra nếu người dùng hỏi giờ
    last_message = messages[-1]['content'].lower() if isinstance(messages[-1], dict) else str(messages[-1]).lower()
    if any(kw in last_message for kw in ["what time is it", "what's the time", "mấy giờ", "giờ hiện tại", "bây giờ là mấy giờ"]):
        now = datetime.datetime.now().strftime("%H:%M:%S")
        return f"The current time is {now}.", 0, 0  # Trả lời ngay, không gọi API
